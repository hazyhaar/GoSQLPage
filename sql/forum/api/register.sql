...
SELECT CASE
    WHEN length($username) < 3 THEN
        '<div class="alert alert-error">Le nom utilisateur doit faire au moins 3 caracteres</div>
    ...
    WHEN length($password) < 8 THEN
        '<div class="alert alert-error">Le mot de passe doit faire au moins 8 caracteres</div>
         <script>setTimeout(() => window.history.back(), 2000);</script>'
    WHEN EXISTS(SELECT 1 FROM forum_users WHERE username = $username) THEN
        '<div class="alert alert-error">Ce nom utilisateur est deja pris</div>
         <script>setTimeout(() => window.history.back(), 2000);</script>'
    WHEN EXISTS(SELECT 1 FROM forum_users WHERE email = $email) THEN
        '<div class="alert alert-error">Cette adresse email est deja utilisee</div>
         <script>setTimeout(() => window.history.back(), 2000);</script>'
    WHEN $terms != 'on' THEN
        '<div class="alert alert-error">Vous devez accepter les conditions</div>
         <script>setTimeout(() => window.history.back(), 2000);</script>'
    ELSE
        '<div class="alert alert-success">Compte cree avec succes ! Redirection...</div>
